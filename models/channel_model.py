...
class AWGN_complex(nn.Module):
    ...
    def get_var(self, snrs, z):
        logdelta = self.snr2logd(snrs)
        delta = torch.exp(logdelta)/torch.sqrt(torch.tensor(2))
        if len(z.shape) > 2:
            delta = delta.unsqueeze(-1).unsqueeze(-1)
        return delta.expand_as(z).to(z.device)
    
if __name__ == "__main__":
    channel = AWGN_complex()
    print(channel.get_var(10))
